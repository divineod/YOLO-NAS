...
def cosine_schedule(
    ...
    if max_steps == 1:
        ...
        decay = (
            ...
            * (np.cos(torch.pi * step / (max_steps - 1)) + 1)
            / 2
        )
    return decay
    ...
    def scale_lr(self, epoch: int) -> float:
        ...
        if epoch < self.warmup_epochs:
            ...
            return cosine_schedule(
                epoch - self.warmup_epochs, self.max_epochs - self.warmup_epochs + 1, 1.0, 0.0
            )
...
if __name__ == '__main__':
    ...
    def testfun(epoch, warmup_epochs, max_epochs):
        if epoch < warmup_epochs:
            return (epoch + 1) / (warmup_epochs + 1)
        else:
            return cosine_schedule(
                epoch - warmup_epochs, max_epochs - warmup_epochs + 1, 1.0, 0.0
        )


    for i in range(total):
        print(i, testfun(i, warmup, total))